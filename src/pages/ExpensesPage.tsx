...
import { ArrowLeft, Plus, CircleDollarSign, AlertCircle, Info, Calculator, FileText, Search, User } from 'lucide-react';
...
export const ExpensesPage: React.FC = () => {
    ...
    // New States for Tabs and Modals
    ...
    const filteredExpenses = expenses.filter(exp => {
        ...
        if (statusFilter !== 'all') {
            // Check if ANY split for this expense is unpaid
    ...
    // calculate theoretical amount of transactions right now vs optimal
                                        ...
                                        return (
                                            <li key={split.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '1rem', background: 'rgba(239, 68, 68, 0.05)', border: '1px solid rgba(239, 68, 68, 0.1)', borderRadius: '12px', marginBottom: '0.75rem' }}>
                                                ...
                                                <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                                                    ...
                                                    <button className="btn-primary" style={{ padding: '0.5rem 1rem', fontSize: '0.875rem', borderRadius: '8px' }} onClick={() => setSelectedExpense(exp || null)}>
                                                        Zapłać / Detale
                                                    </button>
                                                </div>
                                            </li>
                                    ...
                                    {owedToMeSplits.map(split => {
                                        ...
                                        return (
                                            <li key={split.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '1rem', background: 'rgba(16, 185, 129, 0.05)', border: '1px solid rgba(16, 185, 129, 0.1)', borderRadius: '12px', marginBottom: '0.75rem' }}>
                                                ...
                                                <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                                                    <span style={{ fontWeight: 'bold', fontSize: '1.25rem' }}>{split.amount.toFixed(2)} zł</span>
                                                    <button className="btn-secondary" style={{ padding: '0.5rem 1rem', fontSize: '0.875rem', borderRadius: '8px', opacity: 0.8 }} onClick={() => setSelectedExpense(exp || null)}>
                                                        Oznacz / Szczegóły
                                                    </button>
                                                </div>
                                            </li>
                        ...
                        <div style={{ flex: 1, overflowY: 'auto' }}>
                            <ul style={{ listStyle: 'none', padding: 0 }}>
                                {expenses.slice(0, 5).map(exp => (
                                    <li key={exp.id} onClick={() => setSelectedExpense(exp)} style={{ cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '1rem', padding: '1.25rem 0', borderBottom: '1px solid var(--surface-border)', opacity: 0.9, transition: '0.2s' }} onMouseOver={e => e.currentTarget.style.opacity='1'} onMouseOut={e => e.currentTarget.style.opacity='0.9'}>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem', flexWrap: 'wrap', gap: '1rem' }}>
                        ...
                        <div style={{ display: 'flex', gap: '1rem' }}>
                            <div className="input-group" style={{ position: 'relative' }}>
                                <Search size={16} style={{ position: 'absolute', top: '50%', left: '10px', transform: 'translateY(-50%)', color: 'var(--text-secondary)' }} />
                                <input 
                                    type="text" 
                            ...
                            <div className="input-group" style={{ position: 'relative' }}>
                                <User size={16} style={{ position: 'absolute', top: '50%', left: '10px', transform: 'translateY(-50%)', color: 'var(--text-secondary)' }} />
                                <select 
                                    className="input-field" 
                                    style={{ paddingLeft: '2.5rem', width: '200px', appearance: 'none' }}
                                    value={filterPerson}
                            ...
                            <div className="input-group" style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                                <span style={{ fontSize: '0.875rem', color: 'var(--text-secondary)' }}>Od:</span>
                                <input type="date" className="input-field" value={dateFrom} onChange={e => setDateFrom(e.target.value)} />
                                <span style={{ fontSize: '0.875rem', color: 'var(--text-secondary)' }}>Do:</span>
                                ...
                                {filteredExpenses.map(exp => (
                                    ...
                                    >
                                        <td style={{ padding: '1rem 0.5rem', color: 'var(--text-secondary)', fontSize: '0.875rem' }}>
                                            {new Date(exp.created_at).toLocaleDateString('pl-PL', { day:'2-digit', month:'2-digit', hour:'2-digit', minute:'2-digit'})}
                                        </td>
                                        <td style={{ padding: '1rem 0.5rem', color: 'var(--text-secondary)', fontWeight: 600 }}>
                                            {exp.expense_type || 'Zakupy'}
                                        </td>
                                        <td style={{ padding: '1rem 0.5rem', fontWeight: 500 }}>{exp.title}</td>
                                        <td style={{ padding: '1rem 0.5rem', color: 'var(--text-secondary)' }}>{getMemberName(exp.payer_id)}</td>
                                        <td style={{ padding: '1rem 0.5rem', fontWeight: 600, color: 'var(--text-primary)' }}>{exp.amount.toFixed(2)}</td>
                                        <td style={{ padding: '1rem 0.5rem', color: 'var(--text-secondary)' }}>
                                            {splits.some(s => s.expense_id === exp.id && !s.is_paid) ? 'Otwarte' : 'Zamknięte'}
                                        </td>
                                    </tr>
                                ))}
                                {filteredExpenses.length === 0 && (
                                    <tr>
                                        <td colSpan={6} style={{ textAlign: 'center', padding: '2rem', color: 'var(--text-secondary)' }}>Jeszcze nic tu nie ma.</td>
                                    </tr>
                                )}
                            </tbody>
                        </table>
