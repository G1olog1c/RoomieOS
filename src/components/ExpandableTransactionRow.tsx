...
import { ChevronDown, Calendar, User, AlignLeft, CheckCircle, Circle, Trash2 } from 'lucide-react';
...
export const ExpandableTransactionRow: React.FC<ExpandableTransactionRowProps> = ({ 
  ...
  const isOpen = splits.some(s => s.expense_id === expense.id && !s.is_paid);
  ...
  return (
    <>
      ...
      >
        ...
        <td className="expenses-history-cell" style={{ padding: '1rem 0.5rem', color: 'var(--text-secondary)', fontWeight: 600 }}>
          <span className="expenses-history-type-pill">{expenseType}</span>
        </td>
        <td className="expenses-history-cell expenses-history-title-cell" style={{ padding: '1rem 0.5rem', fontWeight: 500 }}>{expense.title}</td>
      ...
      {isExpanded && (
        <tr className="expenses-history-row-expanded" style={{ background: 'rgba(99,102,241,0.05)', borderBottom: '1px solid rgba(99,102,241,0.1)' }}>
